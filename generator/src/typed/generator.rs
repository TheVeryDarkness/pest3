...
pub struct Intermediate<'g> {
    ...
    // Track whether the stack may be modified by the grammar node outside positive or negative
    // predicates.
    //
    // Save snapshots only when the stack may be modified.
    // pub modify_stack: bool,
}
